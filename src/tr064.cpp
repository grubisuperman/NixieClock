...
#define SERIAL_DEBUG_OUT false

// DONT FORGET TO INIT!
void TR064::init(int port, String ip, String user, String pass)
{
  _port = port;
  _ip = ip;
  _user = user;
  ...
  if (SERIAL_DEBUG_OUT)
    Serial.println("Your secret is hashed: " + _secretH);
}
...
void TR064::initNonce()
{
  ...
  if (SERIAL_DEBUG_OUT)
    Serial.print("Got the initial nonce: " + _nonce + " and the realm: " + _realm + "\n");
}
...
String TR064::httpRequest(String url, String xml, String soapaction)
{
  ...
  if (SERIAL_DEBUG_OUT)
    Serial.print("[HTTP] begin: " + _ip + ":" + _port + url + "\n");
  ...
  if (xml != "")
  {
    ...
    if (SERIAL_DEBUG_OUT)
      Serial.print("[HTTP] POST... SOAPACTION: " + soapaction + "\n");
  }
  else
  {
    httpCode = http.GET();
